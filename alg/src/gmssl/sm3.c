...
void gm_sm3_update(SM3_CTX *ctx, const uint8_t *data, size_t data_len)
{
	size_t blocks;
...
void gm_sm3_finish(SM3_CTX *ctx, uint8_t *digest)
{
	int i;
...
void gm_sm3_digest(const uint8_t *msg, size_t msglen,
	uint8_t dgst[SM3_DIGEST_SIZE])
{
	SM3_CTX ctx;
	gm_sm3_init(&ctx);
	gm_sm3_update(&ctx, msg, msglen);
	gm_sm3_finish(&ctx, dgst);
}
